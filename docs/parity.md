# VTFEditQt Parity Checklist (vs. Windows VTFEdit Reloaded)

Last updated: 2025-12-31

This is a living checklist intended to track feature parity between:

- **Windows**: `VTFEdit/` (WinForms / C++/CLI, “VTFEdit Reloaded”)
- **Qt**: `VTFEditQt/` (cross‑platform)

It’s based on a source scan of the Windows UI strings/handlers (primarily `VTFEdit/VTFEdit.h`, plus `VTFEdit/VTFOptions.h`, `VTFEdit/VTFResources.h`, `VTFEdit/VMTCreate.h`, `VTFEdit/BatchConvert.h`) and the current Qt implementation. Some items may still need runtime verification.

## Legend

- **[DONE]** Implemented in VTFEditQt with comparable behavior.
- **[PARTIAL]** Implemented, but missing notable behavior/UI, or workflow differs.
- **[MISSING]** Not implemented in VTFEditQt.
- **[UNKNOWN]** Not confirmed (needs testing or deeper audit).

## File / Project

- **[DONE]** Create new VTF (from image import) + create options (`VTFEditQt/CreateVtfDialog.*`).
- **[DONE]** Create new VMT (basic) (`VTFEditQt/CreateVmtDialog.*`).
- **[DONE]** Open VTF / open VMT (`VTFEditQt/MainWindow.*`).
- **[DONE]** Recent VTF/VMT lists + clear recent (`VTFEditQt/MainWindow.*`).
- **[DONE]** Save VTF / Save As VTF (`VTFEditQt/MainWindow.*`).
- **[DONE]** Save VMT / Save As VMT (`VTFEditQt/MainWindow.*`).
- **[DONE]** Drag & drop `.vtf`, `.vmt`, and images (`VTFEditQt/MainWindow.*`).
- **[DONE]** “Recent Files” parity:
  - Qt now exposes a single `Open Recent Files` menu that mixes `.vtf` + `.vmt` in MRU order, with a single clear action and a configurable limit (default **8**, max **32**) (`VTFEditQt/MainWindow.*`).
...
- **[DONE]** “Mask” toggle parity:
  - Qt “Mask” (Ctrl+W) now toggles **alpha compositing** of the current channel view over the selected background, matching Windows semantics (`VTFEditQt/MainWindow.*`).
...
- **[DONE]** Windows parity mode:
  - Qt exposes a `VTFLib (Windows)` HDR preview mode that uses `VTFLib`’s FP16 exposure (`VTFLIB_FP16_HDR_EXPOSURE`) on the same **0.00–80.00** scale as Windows VTFEdit (default **20.00**) (`VTFEditQt/MainWindow.*`, `VTFEdit/VTFEdit.h`).
...
- **[DONE]** “Information Resource” UI parity:
  - Qt now exposes an **Information** editor for the KVD resource (reads/writes the Windows `"Information"` schema) while still retaining a raw editor (`VTFEditQt/VtfResourcesDialog.*`).
...
- **[DONE]** “Create VMT File” parity:
  - Windows has a dedicated, feature-rich “Create VMT File” dialog (`VTFEdit/VMTCreate.*`).
  - Qt mirrors the Windows dialog’s common texture slots/flags (including `$envmapmask`, `$detail`, `$normalmap`, `$dudvmap`, secondary base/bump, `%tooltexture`, `%keywords`, and the common boolean flags) (`VTFEditQt/CreateVmtDialog.*`).

## Batch Convert

- **[DONE]** Convert folder (batch convert) workflow exists in Qt.
- **[DONE]** UI parity:
  - Qt batch convert now supports an optional **alpha format** selected per-image based on detected transparency, and can optionally create LOD clamp + Information (KVD) resources during conversion (`VTFEditQt/BatchConvertDialog.*`, `VTFEditQt/BatchConvertRunDialog.*`).

## App Options / Misc

- **[DONE]** Global “Options” dialog parity:
  - Qt has a unified options dialog (Tools → Options → Global Options…) and uses it as the source of global defaults/templates (`VTFEditQt/OptionsDialog.*`).
  - Qt now mirrors the Windows format list (27 entries) and exposes “Texture Type” (Animated / Environment Map / Volume Texture) in both Global Options and the Create VTF Options dialog (`VTFEditQt/OptionsDialog.*`, `VTFEditQt/CreateVtfDialog.*`, `VTFEdit/VTFOptions.h`).
...
- **[DONE]** WAD convert:
  ...
  - Qt provides a working WAD2/WAD3 → VTF conversion workflow via **Tools → WAD Convert…** (`VTFEditQt/WadConvertDialog.*`, `VTFEditQt/WadFile.*`).
